...
int do_tm = 0; /* thread mapping */
static int lmap_fac = 2;
...
static inline
struct mem_s* lmap_get_mem(unsigned long address)
{
	return &mem[hash_32(address, lmap_mem_hash_bits)];
}

static inline
struct mem_s* lmap_get_mem_init(unsigned long address)
{
...
int lu_numa_migrate_prep(struct page *page, struct vm_area_struct *vma,
...
int lu_do_numa_page(struct mm_struct *mm, struct vm_area_struct *vma,
