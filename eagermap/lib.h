...
struct lmap_comm_matrix {
	unsigned *matrix;
	spinlock_t lock;
	uint32_t nthreads;
};

typedef struct lmap_comm_matrix comm_matrix_t;
static int max_threads_bits = ilog2(MAX_THREADS);

static inline
unsigned get_matrix(struct lmap_comm_matrix *m, int i, int j)
{
...
static inline
void set_matrix(struct lmap_comm_matrix *m, int i, int j, unsigned v)
{
